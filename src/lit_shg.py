...
class LitSHG(pl.LightningModule):
    ...
    def __init__(self, num_classes, version, lr=1e-3, patience=20, batch_size=16, transfer=True, tune_fc_only=True):
        ...
        self.model = FAN(num_modules=1, num_landmarks=num_classes)
...
class FAN(nn.Module):
    def __init__(self, num_modules=1, end_relu=False, gray_scale=False, num_landmarks=68):
        ...
        # Base part
        if self.gray_scale:
            self.conv1 = CoordConvTh(x_dim=256, y_dim=256, with_r=True, with_boundary=False, in_channels=3, out_channels=64, kernel_size=7, stride=1, padding=3)
        else:
            self.conv1 = CoordConvTh(x_dim=256, y_dim=256, with_r=True, with_boundary=False, in_channels=3, out_channels=64, kernel_size=7, stride=1, padding=3)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_modules):
            hg, _ = self._modules['m' + str(i)](previous, tmp_out)
            ll = hg
...
class HourGlass(nn.Module):
    def __init__(self, num_modules, depth, num_features, first_one=False):
        ...
        self.coordconv = CoordConvTh(x_dim=128, y_dim=128, with_r=True, with_boundary=True, in_channels=256, first_one=first_one, out_channels=256, kernel_size=1, stride=1, padding=0)
...
class ConvBlock(nn.Module):
    def __init__(self, in_planes, out_planes):
        ...
        self.conv2 = self.conv3x3(int(out_planes / 2), int(out_planes / 4), padding=1, dilation=1)
        self.bn3 = nn.BatchNorm2d(int(out_planes / 4))
        self.conv3 = self.conv3x3(int(out_planes / 4), int(out_planes / 4), padding=1, dilation=1)
    ...
    def conv3x3(self, in_planes, out_planes, strd=1, padding=1, bias=False, dilation=1):
